...
<li class="breadcrumb-item{if !$sflink} active{/if}" data-idx="{$idx}" data-separator="{$outputSeparator|htmlentities}">
    {if $sflink}
        <a href="{$link}">{$menutitle}</a>
    {else}
        {$menutitle}
    {/if}
</li>{if $sflink}{$outputSeparator}<li class="active breadcrumb-item" data-idx="{++$idx}">
