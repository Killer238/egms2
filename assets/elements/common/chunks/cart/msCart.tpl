        ...
        <div class="table-responsive">
            <table class="table table-striped">
                {*<tr class="header">
                    ...
                    <th class="weight">{'ms2_cart_price' | lexicon}</th>
                    <th class="remove"></th>
                </tr>*}
                ...
                {foreach $products as $product}
                    {var $image}
                    {if $product.thumb?}
                        <img src="{$product.thumb}" width="100" alt="{$product.pagetitle}" title="{$product.pagetitle}"/>
                    {else}
                        <img width="100" src="{'assets_url' | option}components/minishop2/img/web/ms2_small.png"
                        ...
                        <td class="title">
                            <div class="d-md-flex">
                                <div class="image mw-100 pr-3">
                                    {if $product.id?}
                                        <a href="{$regioncatalog}/{$product.id | resource :'uri'}">{$image}</a>
                                    {else}
                                        {$image}
                                    {/if}
                                </div>
                                <div class="title">
                                    {if $product.id?}
                                        <a href="{$regioncatalog}/{$product.id | resource :'uri'}">{$product.pagetitle}</a>
                                    {else}
                                        {$product.name}
                                    {/if}
