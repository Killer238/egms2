...
<div class="article">Арт. 010782</div>
    <div class="col-md-10 form-control-static price mb-2">
        <span class='msoptionsprice-cost msoptionsprice-{$id}'>{number_format($product.price, 0, ',', ' ')} </span> {'ms2_frontend_currency' | lexicon}
        {if ($default.old_price > 0)}
        <span class="old_price"><span class='msoptionsprice-old-cost msoptionsprice-{$id}'>{number_format($product.price_old, 0, ',', ' ')} </span> {'ms2_frontend_currency' | lexicon} (Экономия: {(number_format($product.price_diff, 0, ',', ' ')) |number}  {'ms2_frontend_currency' | lexicon})</span>
        {/if}
    </div>
