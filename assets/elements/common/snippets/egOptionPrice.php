...
if (!function_exists('egGetModificationOptions')) {
    function egGetModificationOptions(modX & $modx, $product = null, $uri, $showZeroCount = true)
    {
        ...
        //die($uri);
        $q->sortby("rank");
        $product_cache['name'] = $product->get('longtitle');
        $product_cache['product_uri'] = $uri;
        $product_cache['price'] = $product->get('price');
        $product_cache['old_price'] = $product->get('old_price');

        $product_cache['default'] = array(
            'price' => $product_cache['price'],
            'old_price' => $product_cache['old_price'],
...
$old_price = $product_cache['default']['old_price'];
...
if(isset($_GET[$get_val]))
{
    ...
    $product_cache['name'] = $product_cache['name']." (".$_GET[$get_val].")";
    $product_url .= $product_cache['options'][$_GET[$get_val]]['url'];
    $price = $product_cache['options'][$get_val]['price'];
    $old_price = $product_cache['options'][$get_val]['old_price'];
}else{
    ...
    if($catalog_defatr!='')
    {
        ...
        if (!isset($_GET[$r[0]]))
        {
            ...
            $old_price = $product_cache['options'][$r[1]]['old_price'];
        }
...
$output = $pdoTools->getChunk($tpl, [
    'id'          => $product,
    'name'        => $get_val,
    'product_name' => $product_cache['name'],
